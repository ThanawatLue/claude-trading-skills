#!/usr/bin/env python3
"""
US Market Breadth Analyzer — fast US breadth snapshot via TradingView.

Output schema mirrors `market_breadth_<date>.json` so the dashboard's
existing breadth UI can consume it (with metadata.market = "US").

Usage:
    python3 analyze_us_breadth_tv.py --output-dir reports/
"""
...
def composite_score(b: dict) -> tuple[float, str]:
    ...
    ad_balance = max(0.0, min(100.0, ad_balance))
...
def to_markdown(b: dict, score: float, regime: str, ts: str) -> str:
    ...
    lines = [
        "# US Market Breadth Report (TradingView)",
        f"**Generated:** {ts}  |  **Universe:** {b.get('total_stocks', 0)} US stocks  |  **Source:** TradingView Screener",
        "",
...
def main():
    ...
    print("=" * 60)
    print("US Market Breadth Analyzer (TradingView)")
    print("=" * 60)
    print("Fetching breadth snapshot from TradingView...", end=" ", flush=True)
    # Fetch top 5000 liquid stocks for US breadth calculation
    b = get_us_breadth(min_price=args.min_price, limit=5000)
    ...
    base = os.path.join(args.output_dir, f"us_market_breadth_tv_{ts}")

    # Mirror TH breadth schema so dashboard can render it
    payload = {
        "generated": ts,
        "market": "US",
