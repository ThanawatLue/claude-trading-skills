...
class TestGenerateImprovementBacklog:
    ...
    def test_backlog_with_overconfidence(self):
        ...
        postmortems = []
        base_date = datetime(2026, 3, 1)
        for i in range(20):  # 20 signals, all high confidence, many severe FPs
            outcome = "FALSE_POSITIVE_SEVERE" if i < 15 else "TRUE_POSITIVE"
            ret = -0.08 if i < 15 else 0.02
            postmortems.append(
                {
...
class TestLoadPostmortems:
    ...
    def setup_postmortem_dir(self, tmp_path):
        ...
        # Recent postmortems
        (pm_dir / "pm_recent_1.json").write_text(
            json.dumps(
                {
        ...
        (pm_dir / "pm_recent_2.json").write_text(
            json.dumps(
                {
        ...
        # Older postmortem
        (pm_dir / "pm_old_1.json").write_text(
            json.dumps(
                {
        ...
        (pm_dir / "pm_no_date.json").write_text(
            json.dumps(
                {
        ...
        (pm_dir / "pm_os_error.json").write_text(
            json.dumps({"postmortem_id": "pm_os_error", "some": "data"})
        )  # Added postmortem_id
    ...
    def test_load_postmortems_invalid_json_file(self, setup_postmortem_dir, capsys):
        pm_dir = setup_postmortem_dir
        # Assuming for days_back=7, only pm_recent_1 is recent, and pm_no_date, pm_os_error are always included.
        # pm_invalid should be skipped.
        postmortems = load_postmortems(str(pm_dir), days_back=7)
        ...
        assert "Expecting value" in captured.err or "JSONDecodeError" in captured.err
