...
def test_to_markdown_full_buckets(sample_buckets):
    ...
    assert (
        "| 1 | GROW2 | Finance | 200.00 | 65 | +7.0% | +20.0% | 90.0 |" in markdown_output
    )  # Example row from growth


def test_to_markdown_top_n_limit(sample_buckets):
    # Test with top_n limiting the display, even if buckets have more
    sample_buckets["growth"].append(
        {
            "symbol": "GROW3",
            "name": "Growth Stock 3",
            "sector": "Tech",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"avg_turnover": 0, "price": 10, "avgVolume": 1_000_000},
            20_000_000,
            False,
        ),  # 10M turnover
        ...
        ({}, 20_000_000, False),
        (
            {"price": 10, "avgVolume": 1_000_000, "avg_turnover": None},
            20_000_000,
            False,
        ),  # Should use price*avgVolume, resulting in 10M
        (
            {"price": 100, "avgVolume": 200_000, "avg_turnover": None},
            20_000_000,
            True,
        ),  # Should use price*avgVolume, resulting in 20M
    ],
)
...
def test_in_growth(stock_data, expected_ok, expected_score, monkeypatch):
    monkeypatch.setattr(
        "build_watchlists.has_liquidity",
        lambda s, min_avg_turnover=None: stock_data.get("avg_turnover", 0) >= 20_000_000,
    )
...
def test_main_function_e2e(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "build_watchlists.get_thai_stocks",
        lambda limit: [
    ...
    monkeypatch.setattr(
        sys, "argv", ["build_watchlists.py", "--output-dir", str(output_dir), "--top", "2"]
    )
    ...
    with open(json_files[0]) as f:
        ...
        assert not any(
            s["symbol"] == "ILLIQ.BK" for bucket in data["buckets"].values() for s in bucket
        )
        # Check 'NONE1.BK' is not in any bucket
        assert not any(
            s["symbol"] == "NONE1.BK" for bucket in data["buckets"].values() for s in bucket
        )
