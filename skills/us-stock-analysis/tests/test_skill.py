...
class TestUSStockAnalysis(unittest.TestCase):
    ...
    def run_script(
        self, mock_google_web_search, args_list, mock_return_value=None, side_effect=None
    ):
    ...
    def test_basic_analysis_aapl(self):
        mock_web_search_result = {
            "search_results": [{"title": "AAPL info", "snippet": "some snippet"}]
        }
        result = self.run_script(
            ["--ticker", "AAPL", "--analysis_type", "basic"],
            mock_return_value=mock_web_search_result,
        )
    ...
    def test_fundamental_analysis_nvda(self):
        mock_web_search_result = {
            "search_results": [{"title": "NVDA financials", "snippet": "some financial data"}]
        }
        result = self.run_script(
            ["--ticker", "NVDA", "--analysis_type", "fundamental"],
            mock_return_value=mock_web_search_result,
        )
        self.assertNotIn("error", result)
    ...
    def test_fundamental_analysis_amzn_with_context(self):
        mock_web_search_result = {
            "search_results": [{"title": "AMZN valuation", "snippet": "some valuation data"}]
        }
        result = self.run_script(
            [
                "--ticker",
    ...
    def test_technical_analysis_tsla(self):
        mock_web_search_result = {
            "search_results": [{"title": "TSLA technicals", "snippet": "some chart data"}]
        }
        result = self.run_script(
            ["--ticker", "TSLA", "--analysis_type", "technical"],
            mock_return_value=mock_web_search_result,
        )
        self.assertNotIn("error", result)
    ...
    def test_comprehensive_report_msft(self):
        mock_fundamental_web_search_result = {
            "search_results": [{"title": "MSFT financials", "snippet": "some financial data"}]
        }
        mock_technical_web_search_result = {
            "search_results": [{"title": "MSFT technicals", "snippet": "some chart data"}]
        }

        result = self.run_script(
            ["--ticker", "MSFT", "--analysis_type", "comprehensive"],
            side_effect=[mock_fundamental_web_search_result, mock_technical_web_search_result],
        )
        self.assertNotIn("error", result)
        ...
        self.assertIn(
            "Note: Some parts of the analysis are still placeholder", result["summary"]
        )  # Because the sub-analysis summaries are placeholders
        ...
        self.assertIn(
            "Fundamental analysis for MSFT", result["details"]["fundamental_analysis"]["summary"]
        )
        self.assertEqual(
            result["details"]["fundamental_analysis"]["details"]["raw_search_results"],
            mock_fundamental_web_search_result,
        )

        self.assertEqual(result["details"]["technical_analysis"]["ticker"], "MSFT")
        self.assertIn(
            "Technical analysis for MSFT", result["details"]["technical_analysis"]["summary"]
        )
        self.assertEqual(
            result["details"]["technical_analysis"]["details"]["raw_search_results"],
            mock_technical_web_search_result,
        )

    def test_comparison_analysis_aapl_msft(self):
        mock_aapl_web_search_result = {
            "search_results": [{"title": "AAPL basic info", "snippet": "AAPL data"}]
        }
        mock_msft_web_search_result = {
            "search_results": [{"title": "MSFT basic info", "snippet": "MSFT data"}]
        }

        result = self.run_script(
            ["--ticker", "AAPL", "MSFT", "--analysis_type", "comparison"],
            side_effect=[mock_aapl_web_search_result, mock_msft_web_search_result],
        )
        self.assertNotIn("error", result)
        ...
        self.assertIn(
            "Note: Some individual stock analyses are still placeholder", result["summary"]
        )
    ...
    def test_basic_analysis_empty_web_search_results(self):
        ...
        result = self.run_script(
            ["--ticker", "GOOGL", "--analysis_type", "basic"],
            mock_return_value=mock_web_search_result,
        )
        self.assertNotIn("error", result)
    ...
    def test_fundamental_analysis_empty_web_search_results(self):
        mock_web_search_result = {"search_results": []}  # Empty results
        result = self.run_script(
            ["--ticker", "IBM", "--analysis_type", "fundamental"],
            mock_return_value=mock_web_search_result,
        )
        self.assertNotIn("error", result)
    ...
    def test_technical_analysis_empty_web_search_results(self):
        mock_web_search_result = {"search_results": []}  # Empty results
        result = self.run_script(
            ["--ticker", "SPY", "--analysis_type", "technical"],
            mock_return_value=mock_web_search_result,
        )
        self.assertNotIn("error", result)
    ...
    def test_comprehensive_report_empty_web_search_results(self):
        ...
        result = self.run_script(
            ["--ticker", "MSFT", "--analysis_type", "comprehensive"],
            side_effect=[mock_empty_web_search_result, mock_empty_web_search_result],
        )
        self.assertNotIn("error", result)
        self.assertEqual(result["ticker"], "MSFT")
        self.assertEqual(result["analysis_type"], "comprehensive_report")
        ...
        self.assertIn(
            "No significant results from web search",
            result["details"]["fundamental_analysis"]["summary"],
        )
        self.assertIn(
            "No significant results from web search",
            result["details"]["technical_analysis"]["summary"],
        )
    ...
    def test_comparison_analysis_empty_web_search_results(self):
        ...
        self.assertIn(
            "No significant results from web search",
            result["details"]["individual_stock_results"][0]["summary"],
        )
        self.assertIn(
            "No significant results from web search",
            result["details"]["individual_stock_results"][1]["summary"],
        )
    ...
    def test_fundamental_analysis_web_search_exception(self):
        result = self.run_script(
            ["--ticker", "IBM", "--analysis_type", "fundamental"],
            side_effect=Exception("Web search failed for financials"),
        )
        self.assertNotIn("error", result)
        self.assertEqual(result["ticker"], "IBM")
        self.assertEqual(result["analysis_type"], "fundamental_analysis")
        ...
        self.assertIn(
            "Web search failed for financials", result["details"]["raw_search_results"]["error"]
        )

    def test_technical_analysis_web_search_exception(self):
        result = self.run_script(
            ["--ticker", "SPY", "--analysis_type", "technical"],
            side_effect=Exception("Web search failed for technical data"),
        )
        self.assertNotIn("error", result)
        self.assertEqual(result["ticker"], "SPY")
        self.assertEqual(result["analysis_type"], "technical_analysis")
        ...
        self.assertIn("error", result["details"]["raw_search_results"])
        self.assertIn(
            "Web search failed for technical data", result["details"]["raw_search_results"]["error"]
        )
    ...
    def test_comprehensive_report_web_search_exception(self):
        result = self.run_script(
            ["--ticker", "MSFT", "--analysis_type", "comprehensive"],
            side_effect=[
                Exception("Fundamental search failed"),
                Exception("Technical search failed"),
            ],
        )
        self.assertNotIn("error", result)
        self.assertEqual(result["ticker"], "MSFT")
        ...
        self.assertIn(
            "Note: Some parts of the analysis are still placeholder", result["summary"]
        )  # Due to incomplete implementation and exceptions
        self.assertIn(
            "Error performing fundamental analysis",
            result["details"]["fundamental_analysis"]["summary"],
        )
        self.assertIn(
            "Fundamental search failed", result["details"]["fundamental_analysis"]["summary"]
        )
        self.assertIn(
            "Error performing technical analysis",
            result["details"]["technical_analysis"]["summary"],
        )
    ...
    def test_comparison_analysis_web_search_exception(self):
        result = self.run_script(
            ["--ticker", "AAPL", "MSFT", "--analysis_type", "comparison"],
            side_effect=[Exception("AAPL search failed"), Exception("MSFT search failed")],
        )
        self.assertNotIn("error", result)
        self.assertIn("AAPL", result["tickers"])
        self.assertIn("MSFT", result["tickers"])
        ...
        self.assertIn(
            "Error performing basic info analysis for AAPL",
            result["details"]["individual_stock_results"][0]["summary"],
        )
        self.assertIn(
            "AAPL search failed", result["details"]["individual_stock_results"][0]["summary"]
        )
        self.assertIn(
            "Error performing basic info analysis for MSFT",
            result["details"]["individual_stock_results"][1]["summary"],
        )
        self.assertIn(
            "MSFT search failed", result["details"]["individual_stock_results"][1]["summary"]
        )
