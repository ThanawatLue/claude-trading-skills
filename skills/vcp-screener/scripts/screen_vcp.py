...
def summarize_near_miss(result: dict, reason_override: str | None = None) -> dict:
    """Build a compact reason summary for a trend-template pass that missed final VCP."""
    reasons = []
    vcp = result.get("vcp_pattern") or {}
    volume = result.get("volume_pattern") or {}
    pivot = result.get("pivot_proximity") or {}
    if reason_override:
        reasons.append(reason_override)
    if not result.get("valid_vcp", False):
        if vcp.get("error"):
            reasons.append(vcp["error"])
        else:
            reasons.append("VCP pattern is not valid")
    num_contractions = vcp.get("num_contractions")
    if num_contractions is not None and num_contractions < 2:
        reasons.append(f"only {num_contractions} contraction(s)")
    dry_up_ratio = volume.get("dry_up_ratio")
    if dry_up_ratio is None:
        reasons.append("volume dry-up unavailable")
    elif dry_up_ratio > 1.0:
        reasons.append(f"volume not dry enough ({dry_up_ratio:.2f}x)")
    distance = result.get("distance_from_pivot_pct")
    if distance is None:
        reasons.append("pivot distance unavailable")
    elif not (-8.0 <= distance <= 3.0):
        reasons.append(f"pivot distance {distance:+.1f}%")
    risk_pct = pivot.get("risk_pct")
    if risk_pct is None or risk_pct <= 0:
        reasons.append("risk not computable")
    elif risk_pct > 15.0:
        reasons.append(f"risk {risk_pct:.1f}% > 15%")
    state = result.get("execution_state")
    if state in ("Invalid", "Damaged", "Overextended", "Extended", "Early-post-breakout"):
        reasons.append(f"state is {state}")
    if result.get("wide_and_loose"):
        reasons.append("wide-and-loose base")
    if not reasons:
        reasons.append("ranked below final VCP/actionable gate")
    ...
    return {
        ...
        "trend_score": (result.get("trend_template") or {}).get("score"),
        "contraction_score": vcp.get("score"),
        "volume_score": volume.get("score"),
        "pivot_score": pivot.get("score"),
        "rs_score": (result.get("relative_strength") or {}).get("score"),
        "valid_vcp": result.get("valid_vcp", False),
...
def build_near_misses(
    analyzed_results: list[dict], seed_rows: list[dict] | None = None, limit: int = 25
) -> list[dict]:
    seed_rows = seed_rows or []
    summarized = [
        summarize_near_miss(r)
        for r in analyzed_results
        if not r.get("entry_ready", False) or not r.get("valid_vcp", False)
    ]
    summarized.extend(seed_rows)
    summarized.sort(
        key=lambda r: r.get("composite_score") or r.get("trend_score") or 0, reverse=True
    )
...
def main():
    ...
    preflight_constituents = None
    preflight_completed = False
    try:
        if api_key:
            client = FMPClient(api_key=api_key)
            # test if API key is exhausted
            preflight_constituents = client.get_sp500_constituents()
            preflight_completed = True
            # Do not treat an unset/mock attribute as truthy.  This matters for
            # dependency-injected clients and keeps the FMP integration path
            # deterministic on Windows and in offline test runs.
            if getattr(client, "rate_limit_reached", False) is True:
                print(
                    "WARNING: FMP API rate limit reached. Falling back to Yahoo Finance.",
                    file=sys.stderr,
                )
                client = YFClient()
            else:
    ...
    if args.universe:
        ...
        # Reuse the FMP preflight response.  A second request was wasteful and
        # made the integration path unnecessarily slow on rate-limited plans.
        constituents = (
            preflight_constituents if preflight_completed else client.get_sp500_constituents()
        )
        if not constituents:
            print("FAILED")
    ...
    for sym, likelihood, quote in candidates:
        ...
        if passes_trend_filter(tt_result, args.trend_min_score):
            trend_passed.append((sym, quote, tt_result))
    ...
    results = []
    seed_near_misses = []
    for sym, quote, tt_result in trend_passed:
        hist = candidate_histories.get(sym, [])
            ...
            seed_near_misses.append(
                {
                    ...
                    "trend_score": tt_result.get("score"),
                    "valid_vcp": False,
                    "entry_ready": False,
                    "execution_state": "Stale",
                    "reasons": ["stale price action"],
                }
            )
            continue
    ...
    analyzed_results = list(results)
    near_misses = build_near_misses(analyzed_results, seed_near_misses, limit=max(args.top, 25))
    ...
    if exp_r is not None:
        print()
        print("Strategy Expectancy Calibration:")
        print(f"  Recent Expectancy (last {count} closed trades): {exp_r:+.2f}R")
        if exp_r < 0:
            print(
                "  ⚠️ WARNING: Negative expectancy detected. Suggest raising entry threshold to 85+."
            )
        print()
    ...
    generate_json_report(
        top_results, metadata, json_file, all_results=results, near_misses=near_misses
    )
