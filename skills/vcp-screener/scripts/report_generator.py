...
def generate_markdown_report(
    ...
    if se and se.get("expectancy_r") is not None:
        ...
        if exp_r < 0:
            ...
            lines.append(
                f"> **Negative Expectancy Alert:** This strategy has negative expectancy ({exp_r:+.2f}R) recently."
            )
            lines.append(
                "> It is recommended to raise the composite score threshold to **85+** and reduce position sizing."
            )
        else:
            lines.append("")
            lines.append("> [!NOTE]")
            lines.append(
                f"> **Positive Expectancy:** This strategy has positive expectancy ({exp_r:+.2f}R) recently. Normal parameters apply."
            )
        lines.append("")
