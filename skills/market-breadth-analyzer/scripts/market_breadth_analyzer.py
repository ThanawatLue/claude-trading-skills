...
def _run_thai_analysis(args):
    ...
    for sym, hist in histories.items():
        if not hist:
            continue
        valid_bars = sorted(
            [h for h in hist if h.get("close") is not None and h.get("date")],
            key=lambda x: x["date"],
        )
        if not valid_bars:
            continue
    ...
    set_bars = sorted(
        [b for b in set_idx.get("historical", []) if b.get("close") is not None and b.get("date")],
        key=lambda x: x["date"],
    )
    if not set_bars:
    ...
    for i in range(20, len(raw_breadth_data)):
        ...
        prev_ma200 = (
            sum(r["pct_200"] for r in raw_breadth_data[i - 12 : i - 4]) / 800.0
            if i >= 12
            else ma200
        )
    ...
    dummy_summary = {"Average Peaks (200MA)": "0.75", "Average Troughs (8MA < 0.4)": "0.25"}
    ...
    analysis = {
        ...
        "key_levels": {
            "SET Index": {
                "value": f"{latest_r['S&P500_Price']:.2f}",
                "significance": "Current SET Index Level",
            },
            "SMA8 (50)": {
                "value": f"{latest_r['Breadth_Index_8MA']:.4f}",
                "significance": "Smooth 8-day MA of % stocks above SMA50",
            },
            "SMA8 (200)": {
                "value": f"{latest_r['Breadth_Index_200MA']:.4f}",
                "significance": "Smooth 8-day MA of % stocks above SMA200",
            },
        },
    }
