...
def fetch_breadth_data(url: str = BREADTH_CSV_URL) -> list:
    ...
    for row in rows:
        try:
            ...
            print(
                f"WARNING: Skipping breadth data row due to invalid value: {row}",
                file=sys.stderr,
            )
            continue
    return results
...
def fetch_uptrend_data(url: str = UPTREND_CSV_URL, worksheet: str = "all") -> list:
    ...
    for row in rows:
        ...
        try:
            ...
            print(
                f"WARNING: Skipping uptrend data row due to invalid value: {row}",
                file=sys.stderr,
            )
            continue
    return results
...
def fetch_sector_data(url: str = SECTOR_CSV_URL) -> list:
    ...
    for row in rows:
        try:
            ...
            print(
                f"WARNING: Skipping sector data row due to invalid value: {row}",
                file=sys.stderr,
            )
            continue
    return results
...
def _float_or_none(val) -> Optional[float]:
    ...
    try:
        ...
        # Preserve the row so downstream analysis can distinguish a missing
        # metric from a missing session.  The dataclasses intentionally allow
        # optional numeric fields for partially populated CSV rows.
        return None
