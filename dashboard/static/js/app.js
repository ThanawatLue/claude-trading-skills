...
function renderAutoPaperGate(autoPaper) {
  ...
  el.innerHTML = `
    <div class="signal-mini-grid">
      ...
      <div><span>New / Open cap</span><strong>${_srNum(autoPaper.config?.max_new_positions)} / ${_srNum(autoPaper.config?.max_open_positions)}</strong></div>
    </div>
    <div class="signal-note">High-score override: score ${_srNum(autoPaper.config?.high_score_override_min)}+ can expand open cap to ${_srNum(autoPaper.config?.max_open_positions_with_override)}.</div>
