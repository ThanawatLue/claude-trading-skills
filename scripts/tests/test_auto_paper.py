...
def test_risk_sizing_uses_account_risk_in_board_lots(tmp_path: Path) -> None:
    with signal_ledger.connect(tmp_path / "db.sqlite") as conn:
        _register(conn, entry=10, stop=9, target=12)
        config = auto_paper.AutoPaperConfig(
            market="US",
            min_score=70,
            as_of=date(2026, 7, 1),
    ...
    assert candidates[0]["shares"] == 500
    assert candidates[0]["initial_risk"] == 500
    assert candidates[0]["position_value"] == 5_000


def test_portfolio_heat_blocks_candidate_above_budget(tmp_path: Path) -> None:
    with signal_ledger.connect(tmp_path / "db.sqlite") as conn:
        import sys
        ...
        try:
            ...
            conn.execute(
                """INSERT INTO paper_trade
                   ...
                   VALUES ('OPEN', 'US', 'long', 'open', 10, '2026-07-01T00:00:00+00:00',
                           100, 9, 12, 1900)"""
            )
        finally:
    ...
    assert candidates == []
    assert any("portfolio heat" in reason for reason in diagnostics["skipped"][0]["reasons"])


def test_replacement_review_identifies_weaker_open_position(tmp_path: Path) -> None:
    with signal_ledger.connect(tmp_path / "db.sqlite") as conn:
        import sys
        ...
        try:
            ...
            conn.execute(
                """INSERT INTO paper_trade
                   (symbol, market, side, status, entry_price, entry_at, shares,
                    stop_price, target_price, initial_risk, source_score, unrealized_r, days_held)
                   VALUES ('WEAK', 'US', 'long', 'open', 10, '2026-07-01T00:00:00+00:00',
                           100, 9, 12, 100, 72, -0.2, 2)"""
            )
        finally:
            if str(auto_paper.PAPER_SCRIPT_DIR) in sys.path:
                sys.path.remove(str(auto_paper.PAPER_SCRIPT_DIR))
        ...
        config = auto_paper.AutoPaperConfig(
            ...
            replacement_min_score_gap=10,
            replacement_max_unrealized_r=0.25,
            as_of=date(2026, 7, 1),
        )

        review = auto_paper.replacement_review(conn, config)

    assert review["review_required"] is True
    assert review["proposals"][0]["candidate"]["symbol"] == "STRONG"
    assert review["proposals"][0]["review_position"]["symbol"] == "WEAK"


def test_thai_prices_are_rounded_to_valid_set_ticks(tmp_path: Path) -> None:
    with signal_ledger.connect(tmp_path / "db.sqlite") as conn:
        signal_ledger.register_signal(
            conn,
            signal_ledger.SignalRecord(
                signal_id="sig_th_tick",
                symbol="WHAUP.BK",
                market="TH",
                source_skill="thai-swing-momentum",
    ...
    assert len(candidates) == 1
    assert candidates[0]["entry"] == 7.55
    assert candidates[0]["stop"] == 7.2
    assert candidates[0]["target"] == 7.75


def test_thai_auto_paper_blocks_after_market_close(tmp_path: Path) -> None:
    with signal_ledger.connect(tmp_path / "db.sqlite") as conn:
        signal_ledger.register_signal(
            conn,
            signal_ledger.SignalRecord(
                signal_id="sig_th_closed",
                symbol="PTT.BK",
                market="TH",
    ...
    assert candidates == []
    assert diagnostics["skipped"][0]["reasons"] == ["market closed for new TH entries"]


def test_source_rule_can_raise_score_and_reduce_signal_age(tmp_path: Path) -> None:
    with signal_ledger.connect(tmp_path / "db.sqlite") as conn:
        signal_ledger.register_signal(
            conn,
            signal_ledger.SignalRecord(
                signal_id="sig_momo_low",
                symbol="MOMO.BK",
                market="TH",
                source_skill="thai-swing-momentum",
        ...
        config = auto_paper.AutoPaperConfig(
            ...
            as_of=date(2026, 7, 10),
            now=datetime(2026, 7, 10, 4, 0, tzinfo=timezone.utc),
            source_rules={"thai-swing-momentum": {"min_score": 85, "max_age_days": 0}},
        )

        diagnostics = auto_paper.explain_candidates(conn, config)

    reasons = diagnostics["skipped"][0]["reasons"]
    assert "score 82.0 < source minimum 85" in reasons
    assert "age 1d > source maximum 0d" in reasons
