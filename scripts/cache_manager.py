...
from trading_core.sqlite import connect_sqlite
...
class CacheManager:
    ...
    def _conn(self) -> sqlite3.Connection:
        return connect_sqlite(self.db_path, timeout=60.0, check_same_thread=False)
    ...
    def latest_minute_bar_datetime(self, symbol: str) -> datetime | None:
    ...
    def classify_symbols(self, symbols: list[str]) -> tuple[list[str], list[str], list[str]]:
    ...
    def get_profile(self, symbol: str, ttl_days: int = 7) -> dict | None:
    ...
    def get_profiles_batch(self, symbols: list[str], ttl_days: int = 7) -> tuple[dict, list[str]]:
    ...
    def get_earnings_scan(self, symbol: str, ttl_hours: float = 24.0) -> tuple[bool, dict | None]:
    ...
    def save_earnings_scan(self, symbol: str, result: dict | None) -> None:
    ...
    def get_universe(self, ttl_days: int = 30) -> list[str] | None:
    ...
    def save_universe(self, constituents: list[dict]) -> None:
        ...
        with self._conn() as conn:
            ...
            conn.executemany(
                ...
                [(c["symbol"], c.get("name", ""), c.get("sector", ""), now) for c in constituents],
            )
    ...
    def stats(self) -> dict:
        with self._conn() as conn:
            n_syms = conn.execute("SELECT COUNT(DISTINCT symbol) FROM price_bar").fetchone()[0]
            ...
            oldest = conn.execute("SELECT MIN(date) FROM price_bar").fetchone()[0]
            newest = conn.execute("SELECT MAX(date) FROM price_bar").fetchone()[0]
        return {
