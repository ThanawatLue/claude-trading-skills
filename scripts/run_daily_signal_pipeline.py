...
from trading_core.config import resolve_execution_mode
...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    "auto_paper": {
        ...
        "high_score_override_min": 85.0,
        "max_open_positions_with_override": 8,
        "shares": 100,
...
def _build_auto_config(
    ...
    execution_mode = resolve_execution_mode(paper)
    dry_run = execution_mode != "paper"
    fee_model = paper.get("fee_model") or {}
    transaction_cost_bps = (
        effective_transaction_cost_bps(fee_model)
        if fee_model
    ...
    return auto_paper.AutoPaperConfig(
        ...
        max_open_positions=(
            int(paper["max_open_positions"])
            if paper.get("max_open_positions") is not None
            else None
        ),
        high_score_override_min=(
            float(paper["high_score_override_min"])
            if paper.get("high_score_override_min") is not None
            else None
        ),
        max_open_positions_with_override=(
            int(paper["max_open_positions_with_override"])
            if paper.get("max_open_positions_with_override") is not None
            else None
        ),
        ...
        fee_model=fee_model,
        account_size=float(paper["account_size"]) if paper.get("account_size") else None,
        risk_per_trade_pct=(
            float(paper["risk_per_trade_pct"]) if paper.get("risk_per_trade_pct") else None
        ),
        max_position_pct=float(paper["max_position_pct"])
        if paper.get("max_position_pct")
        else None,
        ...
        fingerprint_block=bool(paper.get("fingerprint_block", True)),
...
def write_reports(result: dict[str, Any], output_dir: str | Path) -> dict[str, str]:
    ...
    lines = [
        ...
        f"- Execution mode: {result.get('execution_mode', 'dry_run')}",
...
def run_pipeline(
    ...
    *,
    ...
    job_store = JobRunStore(db_path)
    execution_mode = resolve_execution_mode(config.get("auto_paper") or {})
    run_id = job_store.start(
        "daily_signal_pipeline",
        market=market,
        metadata={"execution_mode": execution_mode, "run_date": run_date.isoformat()},
    )
    ...
    try:
        ...
        job_store.finish(
            ...
            metadata={
                "execution_mode": execution_mode,
                "opened": auto_result.get("opened", 0),
                "eligible": auto_result.get("eligible", 0),
            },
        )
        return result
    except Exception as exc:
        job_store.fail(run_id, str(exc))
        raise
