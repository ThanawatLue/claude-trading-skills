...
try:
    ...
    print(
        "Please install required dependencies: pip install yfinance pandas apscheduler tradingview-screener"
    )
    sys.exit(1)
    ...
    try:
        with warnings.catch_warnings():
            ...
            df_daily = yf.download(
                yf_symbol, period="2d", interval="1d", auto_adjust=True, progress=False
            )
    ...
    try:
        with warnings.catch_warnings():
            ...
            df_minute = yf.download(
                yf_symbol, period="2d", interval="1m", auto_adjust=True, progress=False
            )
...
def run_daily_fetch_job():
    ...
    print(
        f"[{datetime.now()}] Daily job completed. Successfully fetched: {ok}/{total} in {duration:.1f}s"
    )
...
def run_minute_fetch_job():
    ...
    duration = time.time() - start_time
    print(
        f"[{datetime.now()}] Minute job completed. Successfully fetched: {ok}/{total} in {duration:.1f}s"
    )
...
if __name__ == "__main__":
    ...
    group.add_argument(
        "--schedule", action="store_true", help="Run scheduled fetch jobs (blocking)"
    )
    ...
    if args.once:
        ...
        scheduler.add_job(
            run_minute_fetch_job, "cron", day_of_week="mon-fri", hour="10-17", minute=0
        )

        # 2. Schedule daily bars daily Mon-Fri at 17:00 BKK Time (Market Close)
        scheduler.add_job(run_daily_fetch_job, "cron", day_of_week="mon-fri", hour=17, minute=0)
