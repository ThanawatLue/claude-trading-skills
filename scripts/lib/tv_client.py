...
def _safe_query(builder_fn, label: str, market: str = "TH") -> list[dict]:
    ...
    if not _TV_AVAILABLE:
        logger.warning("tradingview-screener not installed. Run: pip install tradingview-screener")
        return []
    try:
...
def tag_with_memberships(
    stocks: list[dict], memberships: dict[str, set[str]] | None = None
) -> list[dict]:
    """
...
def get_thai_by_sector(sector_name: str, limit: int = 200, exclude_drs: bool = True) -> list[dict]:
...
def get_tv_breadth(market: str = "TH", min_price: float = 1.0, limit: int = 1000) -> dict:
    ...
    return {
        ...
        "new_52w_highs": _count(
            lambda s: (s.get("yearHigh") or 0) > 0 and (s.get("price") or 0) >= s["yearHigh"] * 0.98
        ),
        "new_52w_lows": _count(
            lambda s: (s.get("yearLow") or 0) > 0 and (s.get("price") or 0) <= s["yearLow"] * 1.02
        ),
        ...
        "sector_breakdown": dict(
            sorted(sector_breakdown.items(), key=lambda kv: kv[1], reverse=True)
        ),
    }
