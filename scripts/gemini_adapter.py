    ...
    if gemini_path:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.warning(
                    "Gemini CLI returned non-zero code (%d): %s. Falling back to SDK...",
                    result.returncode,
                    result.stderr,
                )
        except Exception as e:
...
def run_gemini_agent(
    ...
    if gemini_path:
        ...
        try:
            ...
            # Respect Free Tier RPM (15 RPM = 4 seconds per request)
            logger.info(
                "Sleeping 4.5 seconds to respect Free Tier API rate limits in agent mode..."
            )
            time.sleep(4.5)
            ...
            if result.returncode == 0:
                ...
                logger.warning(
                    "Gemini CLI agent returned non-zero code (%d): %s. Falling back to SDK...",
                    result.returncode,
                    result.stderr,
                )
        except Exception as e:
    ...
    try:
        ...
        for turn in range(max_turns):
            ...
            tool_results = []
            logger.info(
                "Gemini agent requested tool calls: %s",
                [(fc.name, dict(fc.args)) for fc in tool_calls],
            )
